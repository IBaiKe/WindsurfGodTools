...
export class ToolCallStreamParser {
  ...
  flush() {
    ...
    if (this.inToolCall) {
      ...
      if (this._incState) {
        const s = this._incState;
        this._incState = null;
        return { text: '', toolCalls: [], toolCallDeltas: [{ index: s.index, flush: true }] };
      }
  ...
  /**
   * Incremental feed — emits tool call deltas as they're being parsed,
   * instead of waiting for </tool_call>. Enables streaming tool_calls
   * with incremental arguments.
   *
   * @returns {{ text: string, toolCalls: Array, toolCallDeltas: Array<{index,id?,name?,argsDelta?}> }}
   */
  feedIncremental(delta) {
    if (!delta) return { text: '', toolCalls: [], toolCallDeltas: [] };
    this.buffer += delta;
    const safeParts = [];
    const doneCalls = [];
    const deltas = [];
    ...
    while (true) {
      ...
      if (this.inToolCall) {
        ...
        if (closeIdx === -1) {
          // Still accumulating — try incremental parse
          if (!this._incState) {
            // Try to extract name from partial JSON
            const nameMatch = this.buffer.match(/"name"\s*:\s*"([^"]+)"/);
            if (nameMatch) {
              const idx = this._totalSeen;
              const id = `call_${idx}_${Date.now().toString(36)}`;
              this._incState = { index: idx, id, name: nameMatch[1], emittedArgs: 0 };
              deltas.push({ index: idx, id, name: nameMatch[1] });
            }
          }
          if (this._incState) {
            // Extract arguments content incrementally
            const argsStart = this.buffer.indexOf('"arguments"');
            if (argsStart !== -1) {
              // Find the value after "arguments": — could be string or object
              const colonIdx = this.buffer.indexOf(':', argsStart + 11);
              if (colonIdx !== -1) {
                const afterColon = this.buffer.slice(colonIdx + 1);
                const trimmed = afterColon.trimStart();
                if (trimmed.length > this._incState.emittedArgs) {
                  const newContent = trimmed.slice(this._incState.emittedArgs);
                  if (newContent) {
                    deltas.push({ index: this._incState.index, argsDelta: newContent });
                    this._incState.emittedArgs = trimmed.length;
                  }
                }
              }
            }
        ...
        if (parsed && typeof parsed.name === 'string') {
          const args = parsed.arguments;
          const argsJson = typeof args === 'string' ? args : JSON.stringify(args ?? {});
          const id = this._incState?.id || `call_${this._totalSeen}_${Date.now().toString(36)}`;
          doneCalls.push({ id, name: parsed.name, argumentsJson: argsJson });
          this._totalSeen++;
        } else {
          safeParts.push(`<tool_call>${body}</tool_call>`);
        }
    ...
    return { text: safeParts.join(''), toolCalls: doneCalls, toolCallDeltas: deltas };
  }
}
