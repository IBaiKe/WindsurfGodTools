...
import { getApiKey, acquireAccountByKey, reportError, reportSuccess, markRateLimited, reportInternalError, updateCapability, getAccountList, isAllRateLimited, estimateCooldown } from '../auth.js';
...
import { detectProvider, getProviderInstance, routeDirectProvider } from '../router.js';
import { lsSemaphore } from '../rate-limit.js';

const HEARTBEAT_MS = 5_000;
const QUEUE_RETRY_MS = 1_000;
const QUEUE_MAX_WAIT_MS = 30_000;

function extractRetryAfterMs(errMessage) {
  const m = errMessage.match(/retry\s*(?:after\s+)?(\d+)\s*s/i);
  if (m) return Math.min(5 * 60_000, Math.max(60_000, Number(m[1]) * 1000));
  return 0;
}
...
export async function handleChatCompletions(body) {
  ...
  // ── 直连厂商路由 — 非 Windsurf 目录模型走直连 API ──
  const directProvider = getProviderInstance(modelKey);
  if (directProvider) {
    log.info(`Chat: direct → ${directProvider.name} model=${displayModel} stream=${stream}`);
    const result = await routeDirectProvider(directProvider, body, stream);
    if (result.status === 200) {
      recordRequest({
        model: displayModel, success: true, durationMs: 0, accountId: null,
        source, credit: 0, tokens: result.body?.usage || null,
      });
    }
    return result;
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    // Pre-flight rate limit check: ask server.codeium.com if this account
    // still has message capacity before burning an LS round trip.
    if (isExperimentalEnabled('preflightRateLimit')) {
      try {
        ...
        if (!rl.hasCapacity) {
          const cooldown = estimateCooldown(acct.tier, rl.maxMessages);
          log.warn(`Preflight: ${acct.email} has no capacity (remaining=${rl.messagesRemaining}), cooldown=${Math.round(cooldown / 1000)}s`);
          markRateLimited(acct.id, cooldown, modelKey);
          continue;
        }
      } catch (e) {
    ...
    await lsSemaphore.acquire();
    let result;
    try {
      result = await nonStreamResponse(
...
async function nonStreamResponse(client, id, created, model, modelKey, messages, cascadeMessages, modelEnum, modelUid, useCascade, apiKey, ckey, poolCtx, emulateTools, toolPreamble, source = 'POST /v1/chat/completions', creditMultiplier = 0) {
  ...
  try {
    ...
    if (isRateLimit) {
      const cooldown = extractRetryAfterMs(err.message) || estimateCooldown();
      markRateLimited(apiKey, cooldown, modelKey);
      err.isRateLimit = true; err.isModelError = true;
    }
...
function streamResponse(id, created, model, modelKey, messages, cascadeMessages, modelEnum, modelUid, useCascade, ckey, emulateTools, toolPreamble, source = 'POST /v1/chat/completions', creditMultiplier = 0) {
  return {
    ...
    async handler(res) {
      ...
      // /opt/windsurf and /root/WindsurfGodTools literals can never slip out even
      ...
      try {
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
          ...
          try {
            ...
            if (isRateLimit) {
              const cooldown = extractRetryAfterMs(err.message) || estimateCooldown();
              markRateLimited(currentApiKey, cooldown, modelKey);
              err.isRateLimit = true; err.isModelError = true;
            }
