/**
 * [INPUT]: handlers/*.js, auth.js, dashboard/api.js, response-store.js
 * [OUTPUT]: HTTP 服务器，路由分发，SSE 流式管道
 * [POS]: 系统入口，所有 HTTP 路由在此注册
...
async function route(req, res) {
  ...
  // OpenAI Responses API — /v1/responses. Codex CLI 和新一代 OpenAI 客户端使用此端点。
  ...
  const respMatch = path.match(/^\/v1\/responses\/([a-zA-Z0-9_-]+)$/);
  if (respMatch && (method === 'GET' || method === 'DELETE')) {
    if (!validateApiKey(extractToken(req))) {
      return json(res, 401, { error: { message: 'Invalid API key', type: 'auth_error' } });
    }
    const rid = respMatch[1];
    if (method === 'GET') {
      const stored = responseStore.get(rid);
      if (!stored) return json(res, 404, { error: { message: 'Response not found', type: 'not_found' } });
      return json(res, 200, stored);
    }
    const deleted = responseStore.delete(rid);
    if (!deleted) return json(res, 404, { error: { message: 'Response not found', type: 'not_found' } });
    return json(res, 200, { id: rid, object: 'response.deleted', deleted: true });
  }
