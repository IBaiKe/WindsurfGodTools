...
/**
 * Estimate cooldown based on account tier and server-reported capacity.
 * Returns ms, clamped to [60s, 5min].
 */
export function estimateCooldown(tier = 'unknown', maxMessages = 0) {
  const rpm = TIER_RPM[tier] || 20;
  if (maxMessages > 0) {
    return Math.min(5 * 60_000, Math.max(60_000, Math.ceil(maxMessages / rpm) * 60_000));
  }
  return Math.min(5 * 60_000, RPM_WINDOW_MS);
}
