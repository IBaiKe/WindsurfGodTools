...
// Detect the actual project root from this module's path.
const _repoRoot = (() => {
  try {
    const thisFile = new URL(import.meta.url).pathname;
    return thisFile.replace(/\/src\/sanitize\.js$/, '');
  } catch { return '/root/WindsurfGodTools'; }
})();
...
if (_repoRoot.length > 10 && _repoRoot !== '/root/WindsurfGodTools') {
  PATTERNS.push([new RegExp(_repoRoot.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '(?:\\/[^\\s"\'`<>)}\\],*;]*)?', 'g'), '[internal]']);
}
// /root/WindsurfGodTools is always redacted regardless of auto-detection
PATTERNS.push([/\/root\/WindsurfGodTools(?:\/[^\s"'`<>)}\],*;]*)?/g, '[internal]']);
