...
src/
  ...
  config.js            - env + defaults (.env overrides, provider API keys)
  ...
  rate-limit.js        - 并发信号量 (globalSemaphore + lsSemaphore) + fetchWithRetry (429 auto-retry, proactive throttle)
  router.js            - 多模型路由：Windsurf LS vs 直连厂商 API
  tool-conversion.js   - 双向工具调用协议转换 (OpenAI ↔ Anthropic ↔ Responses)
  handlers/
    chat.js            - /v1/chat/completions: model routing, retry, tool emulation
    messages.js        - /v1/messages: Anthropic protocol adapter (Claude Code native)
    responses.js       - /v1/responses: OpenAI Responses API (Codex CLI)
    models.js          - /v1/models
    tool-emulation.js  - prompt-level <tool_call> protocol for Cascade (incremental streaming)
  providers/
    base.js            - BaseProvider 基类, shared HTTPS + SSE parsing
    anthropic.js       - 直连 Anthropic /v1/messages API
    openai.js          - 直连 OpenAI /v1/chat/completions API
    gemini.js          - 直连 Google Gemini generateContent API
    openrouter.js      - 直连 OpenRouter (OpenAI 兼容) API
  dashboard/
...
**Request flow (dual-path):**

```
Client → server.js route()
  → handlers/chat.js | messages.js | responses.js
  → router.js detectProvider(modelKey)
     ├── 'windsurf' (default) → auth.js → langserver.js → client.js → gRPC
     └── 'anthropic'|'openai'|'gemini'|'openrouter' → rate-limit.js → providers/*.js → HTTPS
```

Windsurf path: `server.js` → `handlers/chat.js` → account pick
...
Direct provider path: `router.js` checks if model is NOT in Windsurf catalog AND
a provider API key is configured. If so, bypasses LS entirely and calls the provider
API directly via `providers/*.js`. Rate limited by `globalSemaphore` (default 5
concurrent) with automatic 429 retry.

Windsurf LS path concurrency is capped by `lsSemaphore` (default 3, `MAX_LS_CONCURRENT`
env var) to prevent gRPC bursts from overwhelming the LS binary. Pre-flight rate
limit checks (`preflightRateLimit`, default ON) query `CheckUserMessageRateLimit`
before each LS round-trip. When a 429 is received, `markRateLimited` uses adaptive
cooldown (extracted from error message or estimated from tier RPM) instead of a
...
sleep 2
pm2 start src/index.js --name windsurf-api --cwd /path/to/WindsurfGodTools
```
